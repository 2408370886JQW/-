      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-xl border-t border-slate-100 pb-safe rounded-t-[32px] shadow-[0_-4px_30px_rgba(0,0,0,0.05)]">
        <div className="flex justify-around items-end h-20 px-2 max-w-md mx-auto relative">
          {navItems.map((item) => {
            ...
            if (item.isSpecial) {
              return (
                <div key={item.path} onClick={togglePublish} className="relative -top-10">
                  <div className="w-20 h-10 bg-white rounded-t-full shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] flex items-end justify-center pb-2 pointer-events-auto absolute left-1/2 -translate-x-1/2 top-0">
                    <motion.div 
                      animate={isPublishOpen ? { rotate: 45, scale: 1.1 } : { rotate: 0, scale: 1 }}
                      whileTap={{ scale: 0.9 }}
                      className="w-14 h-14 rounded-full bg-slate-900 flex items-center justify-center shadow-xl shadow-slate-900/20 -mb-7 cursor-pointer"
                    >
                      <Plus className="w-7 h-7 text-white" strokeWidth={3} />
                    </motion.div>
                  </div>
                  <div className="h-10 w-20" /> {/* Spacer */}
