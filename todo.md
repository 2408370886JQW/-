...
## 消息页面邀约卡片展示与操作
- [x] 在消息列表中展示已发送的邀约卡片（显示发送状态：待回复/已接受/已拒绝/已过期）
- [x] 在消息列表中展示已收到的邀约卡片（含商家信息、套餐信息、邀请人信息）
- [x] 收到的邀约卡片支持「接受邀约」和「拒绝」操作按钮
- [x] 接受/拒绝后卡片状态实时更新
- [x] 邀约卡片视觉样式与邀约流程中的预览卡片保持一致
- [x] 保持现有消息页面整体样式不变

## 重新设计「一起去」交互逻辑
- [x] 修改「一起去」按钮：点击后跳转到相见Tab（而非独立邀约页面）
- [x] 相见模块顶部新增可关闭的邀约提示区域（显示被邀请用户+文案+关闭按钮）
- [x] 邀约区域文案：邀请谁一起 / 选择一个好去处 / 发送邀请约吧
- [x] 用户可在相见模块正常浏览的同时选择商家和套餐进行邀约
- [x] 点击关闭按钮取消邀约模式，恢复相见模块正常浏览
- [x] 选好商家和套餐后生成邀约卡片并发送
- [x] 不强制弹出商家列表，不强制用户必须邀约

## 邀约流程文案与逻辑调整
- [x] 修改商家列表页顶部邀约横幅文案：从"正在为XXX选择好去处"改为"一起去"语义（如"和XXX一起去的好去处"）
- [x] 修改相见入口页邀约横幅文案：统一为"一起去"语义
- [x] 套餐详情页底部按钮恢复为正常选择按钮（不直接发送邀约）
- [x] 选择套餐后进入独立的邀约确认页（显示邀请对象+商家信息+套餐信息）
- [x] 邀约确认页提供【发送邀约】按钮，发送后进入成功页
- [x] 流程必须为：选套餐 → 确认邀约 → 发送/支付

## 偶遇卡片按钮文案修改
- [x] 将「一起去」按钮文案修改为「约TA相见」（TA大写T小写a）
- [x] 仅修改文案，不调整按钮位置与样式，交互逻辑不变
...
## 邀约历史记录页面
- [x] 在“我的”页面增加“邀约历史”入口（图标+文字）
- [x] 创建邀约历史页面组件（/invite-history路由）
- [x] 展示已发送和已收到的邀约记录（Tab切换）
- [x] 每条记录显示：邀请对象、商家、套餐、状态、时间
- [x] 邀约状态区分：待回复/已接受/已拒绝/已过期
- [x] 支持查看邀约详情
- [x] 注册路由到App.tsx

## 好友卡片增加「约TA相见」按钮
- [x] 在好友列表每个好友卡片上增加「约TA相见」快捷按钮
...
## 订单管理页面
- [x] 在“我的”页面增加“我的订单”入口（图标+文字+订单数徽章）
- [x] 创建订单管理页面组件（/orders路由）
- [x] 展示所有已购买的套餐订单（含商家信息、套餐名称、价格、下单时间）
- [x] 订单状态区分：待使用/已使用/已退款/已过期
- [x] 支持Tab筛选不同状态的订单
- [x] 点击订单卡片展开详情（套餐明细、订单编号、使用说明）
- [x] 注册路由到App.tsx

## 已使用订单评价功能
- [x] 已使用订单详情中增加“去评价”按钮入口
- [x] 创建评价弹窗组件（星级评分+文字评价+标签选择）
- [x] 评价提交后订单状态更新为“已评价”，显示评价内容
- [x] 已评价订单展示评价星级和评价文字
- [x] 评价弹窗支持关闭/取消操作
...
## 订单退款申请功能
- [x] 待使用订单详情中“申请退款”按钮触发退款申请弹窗
- [x] 退款申请弹窗包含：退款原因选择（多个预设原因）+ 文字说明输入
- [x] 退款原因选项：行程变更/不想去了/选错套餐/商家问题/其他原因
- [x] 提交退款申请后订单状态变为“退款中”，显示退款进度
- [x] 退款中的订单展示退款原因和申请时间
- [x] 退款成功后订单状态变为“已退款”
- [x] 退款申请弹窗支持关闭/取消操作

## 退款中订单"取消退款"功能
- [x] 在退款中订单详情中增加"取消退款"按钮
- [x] 点击后弹出确认对话框，确认后取消退款申请
- [x] 取消退款后订单状态恢复为"待使用"，退款信息清除
- [x] 统计卡片实时更新

## 订单页Tab筛选栏数字标签显示修复
- [x] 修复Tab栏中"待使用"、"已使用"、"退款中"、"已退款"旁边的数量数字被遮挡/显示不全的问题
- [x] 确保所有Tab标签的数字完整可见

## 用户卡片"喜欢"功能
- [x] 在用户卡片上增加"喜欢"按钮
- [x] 点击喜欢按钮切换喜欢/取消喜欢状态
- [x] 被喜欢的用户在地图上有特殊展示形式（爱心光晕/特殊边框）

## "我喜欢的人"列表页面
- [x] 在"我的"页面增加"我喜欢的人"入口（图标+文字+喜欢数量徽章）
- [x] 创建喜欢列表页面组件（/liked-users路由）
- [x] 展示所有被喜欢的用户（头像、昵称、性别、在线状态等）
- [x] 支持取消喜欢操作
- [x] 支持点击用户跳转到地图定位该用户
- [x] 注册路由到App.tsx

## "谁喜欢了我"页面
- [x] 创建WhoLikedMePage组件（/who-liked-me路由）
- [x] 用户列表：头像+名字+"喜欢了你"+格式化时间
- [x] 时间格式化规则：<5分钟显示"刚刚"，当前年份显示"月日 时间"，非当前年份显示"年 月日 时间"
- [x] 列表下划展示"喜欢地图"，在地图上展示每个喜欢我的人的位置
- [x] 在"我的"页面增加"谁喜欢了我"入口
- [x] 注册路由到App.tsx

## 消息通知页"喜欢了你"推送提醒
- [x] 在通知页面增加"XXX喜欢了你"类型的通知消息
- [x] 通知展示头像、名字、"喜欢了你"文案和时间
- [x] 点击通知可跳转到"谁喜欢了我"页面

## 底部Tab栏"消息"图标未读红点
- [x] 在底部导航栏"消息"图标上增加未读通知数量红点标记
- [x] 红点显示未读数量，超过99显示99+
- [x] 无未读时不显示红点

## 进入通知页自动清除红点
- [x] 创建全局未读计数Context，让BottomNav和通知页共享状态
- [x] 进入通知页时自动将未读数量归零，红点消失
- [x] 在App.tsx中包裹NotificationProvider
