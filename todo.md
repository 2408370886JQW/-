...
- [x] 为商家增加回复用户评价功能，在订单详情页评价区域下方展示商家公开回复
- [x] 为订单数据增加商家回复字段（回复内容、回复时间、商家名称）
- [x] 商家回复UI样式与用户评价区分（不同背景色、商家标识）
- [x] 在距离排序模式下为商家卡片增加距离渐变色条指示器
- [x] 距离最近的商家卡片高亮显示，距离越远颜色越淡
- [x] 距离信息在排序模式下醒目突出展示
- [x] 修复距离排序模式下排名徽章（#1-#4）遮挡店名的位置问题

## 偶遇卡片新增「TA的动态」展示区
- [x] 在偶遇卡片星座/兴趣标签下方新增个人动态展示区
- [x] 动态以横向列表形式展示，每条含动态图片缩略图+点赞图标+点赞数
- [x] 默认仅展示前3条个人动态
- [x] 超过3条时在第3条下方显示「查看更多」入口
- [x] 点击查看更多进入完整动态列表页
- [x] 视觉样式保持原有白色卡片风格，仅参考参考图的结构逻辑
- [x] 回退深色卡片样式改动，恢复原有白色卡片风格
- [x] 仅在标签下方结构性新增动态展示区，不改变整体视觉风格

## 偶遇卡片动态点赞位置调整
- [x] 将点赞图标+点赞数从缩略图下方移到缩略图卡片内部右下角
- [x] 点赞信息作为动态内容的一部分叠加在图片上，不独立悬浮
- [x] 其余布局和视觉风格保持不变

## 偶遇卡片动态「查看更多」交互调整
- [x] 删除底部「查看更多动态」按钮
- [x] 当动态超过3条时，在第3条右侧增加「查看更多」入口卡片（横向滑动时可见）
- [x] 点击「查看更多」入口卡片进入动态详情页
- [x] 创建动态详情页，展示该用户的完整动态列表
- [x] 动态详情页支持返回地图页面

## 动态详情页评论功能
- [x] 为每条动态添加mock评论数据（头像、昵称、内容、时间、点赞数）
- [x] 在每条动态下方展示评论列表
- [x] 底部固定评论输入框+发布按钮
- [x] 支持发布新评论并即时显示在列表中
- [x] 评论支持点赞交互
...
## 动态详情页图片折叠交互（小红书风格）
- [x] 向上滑动进入评论区时，图片区域自动收起/折叠
- [x] 评论区占据主要可视区域，可连续查看全部评论
- [x] 向下滑回顶部时，图片区域恢复显示（点击展开图片按钮）
- [x] 页面整体样式保持不变，仅调整滚动与收起逻辑

## 偶遇卡片新增「一起去」按钮及邀约流程
- [x] 在偶遇卡片「打招呼」与「查看主页」按钮之间新增「一起去」按钮
- [x] 点击「一起去」进入邀约流程页面，顶部显示「邀请XXX一起」
- [x] 邀约流程第一步：选择商家（搜索+分类筛选+商家列表）
- [x] 邀约流程第二步：选择对应套餐
- [x] 选择完成后生成邀约卡片（含被邀请用户信息、商家信息、套餐信息、有效期）
- [x] 邀约卡片包含「接受邀约」和「换一个」两个按钮
- [x] 点击发送将邀约卡片发送给被邀请用户，显示发送成功页
- [x] 邀约状态：待回复→已接受/已拒绝/已过期(24h)
- [x] 保持现有页面整体样式不变
- [x] 发送成功页支持返回地图或查看消息
